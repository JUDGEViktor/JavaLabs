...
class Manager extends ManagerGrammar {
    ...
    private RC LinkPipeLineElements(){
        ...
        for(int i = 0; i < pipeLineElements.length; i++){
            if(i > 0){
                RC errorCode = pipeLineElements[i].setProducer(pipeLineElements[i-1]);
                if(errorCode != RC.CODE_SUCCESS)
                    return errorCode;
            }
            if(i < pipeLineElements.length - 1){
                RC errorCode =  pipeLineElements[i].setConsumer(pipeLineElements[i+1]);
                if(errorCode != RC.CODE_SUCCESS)
                    return errorCode;
            }
        }
